...
var events = []Event{
	...
	{
		Key:                 "email.billing.trial_ending.v1",
		Domain:              "billing",
		TriggerSource:       "Stripe customer.subscription.trial_will_end or managed short-trial activation",
		Provider:            "loops",
		TemplateEnv:         "LOOPS_BILLING_TRIAL_ENDING_TRANSACTIONAL_ID",
		LoopsEventName:      "billing_trial_ending",
		ExternalLoopsConfig: "Audit workflows listening to billing_trial_ending before enabling this transactional template.",
		DeliveryClass:       CriticalTransactional,
		PreferenceCategory:  EssentialAccountBilling,
		CanUnsubscribe:      false,
		PreferenceGated:     false,
		...
		FooterPolicy:        FooterRequiredNotice,
		RecipientPolicy:     "workspace owner",
		IdempotencyPolicy:   "billing_trial_ending:{trial_grant_id}:{trial_end}",
		RequiredVariables:   []string{"workspace_name", "plan_id", "plan_name", "trial_end", "days_remaining", "post_trial_price", "billing_url", "cancel_url"},
		AuditPolicy:         "record one send attempt per trial grant and end time",
		FallbackPolicy:      "send immediately after activation for one-, two-, or three-day trials",
		RetentionPolicy:     "retain metadata and variable snapshots for 13 months",
		OwnerArea:           "Billing",
	},
	{
	...
	{
		Key:                 "email.quota.paid_plan_warning.v1",
		Domain:              "quota",
		TriggerSource:       "paid workspace crosses the 80% or 90% effective-usage threshold",
		Provider:            "loops",
		TemplateEnv:         "LOOPS_PAID_PLAN_QUOTA_TRANSACTIONAL_ID",
		ExternalLoopsConfig: "No Loops workflow should independently calculate paid quota thresholds.",
		DeliveryClass:       ServiceAlert,
		LoopsEventName:      "paid_plan_quota_warning",
		PreferenceCategory:  UsageQuotaAlerts,
		CanUnsubscribe:      false,
		PreferenceGated:     true,
		FooterPolicy:        FooterManagePreferences,
		RecipientPolicy:     "workspace owner",
		IdempotencyPolicy:   "paid_plan_quota:{workspace_id}:{period}:{threshold_percent}",
		RequiredVariables: []string{
			"subject",
			"preview_text",
			"headline",
		...
		AuditPolicy:     "use paid_plan_quota_notifications ledger and email_send_attempts",
		FallbackPolicy:  "none by default",
		RetentionPolicy: "retain ledger and variable snapshots for 13 months",
		OwnerArea:       "Billing / Growth lifecycle",
	},
	{
		Key:                 "email.quota.paid_plan_alert.v1",
		Domain:              "quota",
		TriggerSource:       "paid workspace crosses a required 100% through 120% effective-usage threshold",
		Provider:            "loops",
		TemplateEnv:         "LOOPS_PAID_PLAN_QUOTA_TRANSACTIONAL_ID",
		ExternalLoopsConfig: "No Loops workflow should independently calculate paid quota thresholds.",
		DeliveryClass:       ServiceAlert,
		LoopsEventName:      "paid_plan_quota_alert",
		PreferenceCategory:  UsageQuotaAlerts,
		CanUnsubscribe:      false,
		PreferenceGated:     false,
		RequiredReason:      "Quota alerts at and above 100% are required service notices about scheduling availability and account operation.",
		FooterPolicy:        FooterRequiredNotice,
		RecipientPolicy:     "workspace owner",
		IdempotencyPolicy:   "paid_plan_quota:{workspace_id}:{period}:{threshold_percent}",
		RequiredVariables: []string{
	...
	{
		Key:                 "email.publishing_restriction.restriction_notice.v1",
		Domain:              "publishing",
		TriggerSource:       "Super Admin separately previews and irreversibly confirms a restriction notice campaign",
		Provider:            "loops",
		TemplateEnv:         "LOOPS_TIKTOK_FREE_RESTRICTION_NOTICE_TRANSACTIONAL_ID",
		ExternalLoopsConfig: "No workflow may auto-send this notice from a restriction toggle or contact property.",
		DeliveryClass:       ServiceAlert,
		PreferenceCategory:  PublishingFailures,
		CanUnsubscribe:      false,
		PreferenceGated:     false,
		RequiredReason:      "Operational notice about temporary publishing availability for an affected plan.",
		FooterPolicy:        FooterRequiredNotice,
		RecipientPolicy:     "deduped current Free workspace owners with active connected TikTok, snapshotted at confirmation and rechecked at send",
		IdempotencyPolicy:   "{cycle_id}:restriction_notice:{canonical_user_id}",
		RequiredVariables:   []string{"subject", "body"},
		AuditPolicy:         "pre-render first_name, then create/link one email_send_attempts row before every network attempt via loops.AuditedClient",
		FallbackPolicy:      "none",
		RetentionPolicy:     "retain campaign snapshot and audited send metadata for the operational audit period",
		OwnerArea:           "Publishing / Operations",
	},
	{
		Key:                 "email.publishing_restriction.recovery_notice.v1",
		Domain:              "publishing",
		TriggerSource:       "Super Admin separately previews and irreversibly confirms a recovery notice campaign",
		Provider:            "loops",
		TemplateEnv:         "LOOPS_TIKTOK_FREE_RECOVERY_NOTICE_TRANSACTIONAL_ID",
		ExternalLoopsConfig: "No workflow may auto-send recovery from a restriction toggle or contact property.",
		DeliveryClass:       ServiceAlert,
		PreferenceCategory:  PublishingFailures,
		CanUnsubscribe:      false,
		PreferenceGated:     false,
		RequiredReason:      "Operational recovery notice limited to recipients successfully notified in the same restriction cycle.",
		FooterPolicy:        FooterRequiredNotice,
		RecipientPolicy:     "same-cycle successfully notified owners who remain Free with active connected TikTok",
		IdempotencyPolicy:   "{cycle_id}:recovery_notice:{canonical_user_id}",
		RequiredVariables:   []string{"subject", "body"},
		AuditPolicy:         "pre-render first_name, then create/link one email_send_attempts row before every network attempt via loops.AuditedClient",
		FallbackPolicy:      "none",
		RetentionPolicy:     "retain campaign snapshot and audited send metadata for the operational audit period",
