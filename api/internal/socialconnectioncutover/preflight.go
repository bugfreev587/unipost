...
WITH source AS (
  ...
  SELECT
    ...
    COUNT(DISTINCT external_user_id) FILTER (WHERE external_user_id IS NOT NULL AND is_live) AS managed_owner_count,
    COUNT(*) FILTER (WHERE connection_type = 'managed' AND external_user_id IS NULL AND is_live) AS missing_managed_owner_count,
    COUNT(*) FILTER (WHERE connection_type = 'byo' AND external_user_id IS NOT NULL AND is_live) AS byo_owner_count,
