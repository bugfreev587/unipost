...
func TestPostDeliveryJobConnectionSnapshotMigrationContract(t *testing.T) {
	source, err := os.ReadFile("migrations/139_delivery_job_connection_snapshot.sql")
	if err != nil {
...
func TestPostPhysicalTargetBatchReservationContract(t *testing.T) {
	migration, err := os.ReadFile("migrations/139_delivery_job_connection_snapshot.sql")
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
...
func TestDailyPublishReservationUsesPhysicalConnectionAtomically(t *testing.T) {
	...
	migration, err := os.ReadFile("migrations/141_physical_daily_publish_reservations.sql")
	if err != nil {
		t.Fatal(err)
	}
