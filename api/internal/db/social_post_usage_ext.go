...
WITH updated_result AS (
  ...
  RETURNING id, post_id, social_account_id, status, external_id, error_message,
    ...
    x_credits_counted, x_credit_operation, x_credit_catalog_version,
    x_credit_billing_mode
),
usage_increment AS (
...
SELECT
  ...
  r.x_credits_counted, r.x_credit_operation, r.x_credit_catalog_version,
  r.x_credit_billing_mode
