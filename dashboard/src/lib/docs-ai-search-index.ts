...
export const DOCS_AI_INDEX: DocsAiChunk[] = [
  ...
  chunk({
    ...
    content:
      "UniPost directly supports GIF publishing to X and Facebook. Publish a hosted GIF with platform_posts[].media_urls, or reserve a local image/gif upload with POST /v1/media, PUT bytes to upload_url, poll GET /v1/media/{media_id}, then validate and publish with media_ids. A GIF sent to both X and Facebook must be 5 MB or smaller. LinkedIn and Threads native GIF integration is coming soon. For video-only destinations, POST /v1/media/gif-conversions converts an uploaded GIF to universal_mp4_v1; poll GET /v1/media/gif-conversions/{id} and publish output_media_id as normal video media. Conversion and publishing are separate operations.",
  }),
  chunk({
    id: "api-gif-conversions",
    title: "Convert GIF to MP4",
    path: "/docs/api/media/gif-conversions",
    section_id: "endpoint",
    primary_nav: "API Reference",
    section_title: "GIF conversion API",
    product_area: "publishing",
    tags: ["GIF to MP4", "gif conversion", "universal_mp4_v1", "media processing", "background color", "idempotency"],
    intent_tags: ["posting", "reference"],
    endpoint_aliases: [
      "POST /v1/media/gif-conversions",
      "GET /v1/media/gif-conversions/{id}",
    ...
    platforms: ["instagram", "tiktok", "pinterest", "youtube", "bluesky"],
    content:
      "POST /v1/media/gif-conversions accepts an owned uploaded image/gif media ID up to 50 MB and an optional six-digit background_color. It returns an asynchronous gif_to_mp4 job using the fixed universal_mp4_v1 profile. Poll GET /v1/media/gif-conversions/{id}; on success, output_media_id is a silent H.264 yuv420p constant-30-FPS fast-start MP4. Idempotent replay does not consume another rolling fair-use unit. Active capacity is shared with Audio Overlay, and Plan retention protects active inputs then cleans terminal inputs and outputs after their assigned deadline. The endpoint never publishes automatically.",
  }),
  chunk({
