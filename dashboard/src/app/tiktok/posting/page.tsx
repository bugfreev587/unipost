...
import { TikTokReviewPostingClient, type ReviewSession } from "./tiktok-review-posting-client";
...
export default async function TikTokReviewPostingPage({ searchParams }: PageProps) {
...
async function loadReviewSession(token: string): Promise<{ session: ReviewSession | null; error: string }> {
  if (!token) {
  ...
  try {
    ...
    const body: ApiEnvelope<ReviewSession> = await res.json();
    if (!res.ok || !body.data) {
