...
const CSS = `
:root{--blog-bg:#050506;--blog-s1:#111316;--blog-s2:#171a1f;--blog-s3:#0d1117;--blog-border:#252a33;--blog-b2:#343b47;--blog-text:#f6f7fb;--blog-muted:#a3a8b4;--blog-muted2:#7f8797;--blog-accent:#20c997;--blog-blue:#23a8ff;--blog-shadow:0 24px 80px rgba(0,0,0,.35);--blog-r:8px;--blog-mono:var(--font-fira-code),monospace;--blog-ui:var(--font-dm-sans),system-ui,sans-serif;--blog-content-max:1560px;--blog-article-max:820px;--blog-px:32px;--marketing-text:#f7f7f5;--marketing-muted:#a3a8b4;--marketing-surface:#050506;--marketing-surface-alt:#111316;--marketing-border:rgba(255,255,255,.08);--marketing-border-strong:rgba(255,255,255,.14);--platform-icon-mono:#f6f7fb}
*{box-sizing:border-box}
body{background:var(--blog-bg);color:var(--blog-text);font-family:var(--blog-ui);font-size:15px;line-height:1.6;-webkit-font-smoothing:antialiased}
.mk-nav{background:#050506;border-bottom:1px solid rgba(255,255,255,.08)}
.mk-nav-link{color:#a3a8b4}
.mk-nav-link:hover,.mk-nav-link.active{color:#f7f7f5;background:#15181d}
.mk-auth-btn-ghost{color:#c2c7d0;border-color:rgba(255,255,255,.14)}
.mk-auth-btn-ghost:hover{background:#15181d;color:#fff}
...
.lp-btn-primary{background:#2563eb;color:#fff}.lp-btn-primary:hover{background:#1d4ed8;box-shadow:0 16px 45px rgba(37,99,235,.28)}
.lp-btn-outline{background:transparent;color:var(--blog-text);border-color:var(--blog-b2)}.lp-btn-outline:hover{background:var(--blog-s2);border-color:#465063}
.lp-btn-lg{padding:12px 28px;font-size:15px;border-radius:10px}
.blog-shell{min-height:100vh;background:var(--blog-bg)}
...
.blog-card:hover{border-color:#334155;background:#141820;transform:translateY(-2px)}
...
.blog-tag{display:inline-flex;align-items:center;border:1px solid var(--blog-border);background:#0b0e13;border-radius:999px;padding:5px 10px;font-family:var(--blog-mono);font-size:11px;color:#aeb6c6}
...
.blog-body p{font-size:17px;line-height:1.82;color:#c1c8d4;margin:0 0 24px}
.blog-body .lead{font-size:20px;line-height:1.75;color:#f2f4f8;margin-bottom:28px}
.blog-body h2{font-size:30px;font-weight:800;letter-spacing:0;line-height:1.2;color:var(--blog-text);margin:52px 0 16px}
.blog-body ul{list-style:disc;margin:4px 0 30px;padding-left:22px;color:#c1c8d4}
.blog-body li{font-size:16.5px;line-height:1.72;margin-bottom:10px;padding-left:4px}
.blog-code{background:#0b1120;border:1px solid #1e293b;border-radius:12px;padding:22px;overflow:auto;margin:28px 0 34px;box-shadow:var(--blog-shadow)}
.blog-code code{font-family:var(--blog-mono);font-size:13px;line-height:1.75;color:#dbeafe;white-space:pre}
.blog-note{background:#0f141b;border:1px solid var(--blog-border);border-left:3px solid var(--blog-accent);border-radius:10px;padding:20px 22px;margin:30px 0}
.blog-note-title{font-size:14px;font-weight:800;color:var(--blog-text);margin-bottom:7px}.blog-note p{font-size:15px;line-height:1.72;margin:0;color:#c1c8d4}
.blog-body a{color:var(--blog-blue);text-decoration:none;border-bottom:1px solid rgba(35,168,255,.32)}.blog-body a:hover{color:#7dd3fc;border-bottom-color:#7dd3fc}
.blog-body p code,.blog-body li code,.blog-body td code,.blog-summary code,.blog-faq code{font-family:var(--blog-mono);font-size:13px;background:#0b1120;border:1px solid #1e293b;border-radius:5px;padding:1px 6px;color:#cbd5e1}
.blog-summary{background:#0b1320;border:1px solid #1f2c44;border-radius:14px;padding:22px 24px;margin:0 0 36px;box-shadow:var(--blog-shadow)}
...
.blog-summary ul{list-style:disc;margin:0;padding-left:20px}.blog-summary li{font-size:15.5px;line-height:1.7;color:#dbe3ef;margin-bottom:8px}.blog-summary li:last-child{margin-bottom:0}
.blog-table-wrap{margin:30px 0 36px;overflow-x:auto;border:1px solid var(--blog-border);border-radius:12px;background:#0b1120;box-shadow:var(--blog-shadow)}
.blog-table{width:100%;border-collapse:collapse;font-size:14.5px}
.blog-table th,.blog-table td{padding:13px 16px;border-bottom:1px solid var(--blog-border);text-align:left;vertical-align:top;color:#c1c8d4}
.blog-table th{font-weight:800;color:var(--blog-text);background:#0d1424;font-size:12.5px;letter-spacing:.04em;text-transform:uppercase}
...
.blog-faq-item[open]{border-color:#334155;background:#141820}
...
.blog-faq-item p{font-size:15.5px;line-height:1.72;color:#c1c8d4;margin:0 0 16px}
