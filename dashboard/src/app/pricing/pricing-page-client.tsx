...
import { isPlanVisibleToNewUsers } from "@/lib/plan-visibility";
...
const FAQS = [
  { q: "Why Free, Basic, Growth, Team?", a: "Each plan corresponds to a stage of how you use UniPost — evaluating the API, using the dashboard as your operating console, embedding it into your product, or running it as a multi-operator team. Pick by which of those describes you, not by raw post volume." },
  ...
  { q: "Is there a free trial?", a: "The Free plan has no time limit. If your workspace receives a time-limited paid-plan trial offer, sign in and select the named plan here or in Settings → Billing. A payment method is required, billing starts after the trial unless you cancel renewal, and changing plans during a scheduled or active trial ends it immediately." },
  { q: "Why is X (Twitter) not on the Free plan?", a: "The X API has the highest per-call cost of any platform we support, and the Free plan's 100-post quota is too small to absorb that cost without distorting our pricing for everyone else. Free workspaces can read existing X data; new X publishes and connections require a paid plan starting with Basic at $19/mo." },
...
export default function PricingPage() {
  ...
  const [trialActionPlan, setTrialActionPlan] = useState<string | null>(null);
  ...
  const loadPlan = useCallback(async () => {
    ...
    try {
      ...
      if (!token) return;
      const [profiles, billing] = await Promise.all([listProfiles(token), getBilling(token)]);
      if (profiles.data?.[0]?.id) setProjectId(profiles.data[0].id);
      setCurrentPlan(billing.data.plan);
      setTrial(billing.data.trial ?? null);
    } catch {
  ...
  const hasForfeitableTrial = Boolean(
    trial?.changing_plan_forfeits_trial && (trial.status === "scheduled" || trial.status === "active"),
  );

  async function handleTrialPlanAction(planId: string) {
    setTrialActionPlan(planId);
    setTrialActionError(null);
    try {
      const token = await getToken();
      if (!token) throw new Error("Sign in to manage this trial offer.");
      if (trial?.plan_id === planId && (trial.status === "pending_activation" || trial.status === "checkout_pending")) {
        const response = await createCheckout(token, planId);
        window.location.href = response.data.checkout_url;
        return;
      }
      if (hasForfeitableTrial && trial?.plan_id !== planId) {
      ...
      window.location.href = profileId
        ? `${APP_URL}/settings/billing?upgrade=${encodeURIComponent(planId)}`
        : `${APP_URL}/settings/billing`;
    } catch (error) {
      setTrialActionError(error instanceof Error ? error.message : "Unable to update billing.");
    } finally {
      setTrialActionPlan(null);
    }
  }
  ...
  return (
    <>
      ...
      <div className="pr-page">
        ...
        {/* CARDS */}
        {trialActionError ? <div role="alert" className="pr-trial-error"><strong>Trial action failed.</strong> {trialActionError}</div> : null}
        <div className="pr-cards">
          {PUBLIC_TIERS.map((t) => {
            const isCurrent = currentPlan === t.id;
            const isTrialTier = trial?.plan_id === t.id;
            const formattedTrial = isTrialTier && trial ? formatWorkspaceTrial(trial) : null;
            ...
            const ctaLabel = isCurrent
              ...
              : t.price === 0
                ? "Get started"
                : t.cta ?? "Choose plan";
            const trialCta = isTrialTier && trial?.status === "pending_activation"
              ? `Start ${trial.duration_days}-day free trial`
              : isTrialTier && trial?.status === "checkout_pending"
                ? "Continue checkout"
                : isTrialTier && trial?.status === "provisioning"
                  ? "Activation in progress"
                : null;
            const usesTrialMutation = Boolean(
              isSignedIn && t.id !== "free" && (trialCta || (hasForfeitableTrial && !isTrialTier)),
            );
            return (
              <div key={t.id} className={`pr-card ${t.highlight ? "hi" : ""} ${isCurrent ? "current" : ""}`}>
                ...
                {formattedTrial ? (
                  <div className="pr-trial" aria-label={`${t.name} managed trial`}>
                    <div className="pr-trial-head">
                      <span className="pr-trial-title">{formattedTrial.headline}</span>
                      <span className="pr-trial-badge">{formattedTrial.badge.label}</span>
                    </div>
                    {formattedTrial.timeline.map((item) => (
                      <div className="pr-trial-line" key={`${item.label}-${item.value}`}>
                        <span>{item.label}</span><span>{item.value}</span>
                      </div>
                    ))}
                    {formattedTrial.remainingDays !== null ? (
                      <div className="pr-trial-warning">{formattedTrial.remainingDays} day{formattedTrial.remainingDays === 1 ? "" : "s"} remaining.</div>
                    ) : null}
                  </div>
                ) : null}
                {usesTrialMutation ? (
                  <button
                    type="button"
                    className={`pr-btn pr-btn-tier ${t.highlight ? "pr-btn-tier-hi" : ""}`}
                    onClick={() => void handleTrialPlanAction(t.id)}
                    disabled={trialActionPlan !== null || trial?.status === "provisioning"}
                    aria-busy={trialActionPlan === t.id}
                  >
                    {trialActionPlan === t.id ? "Working…" : trialCta ?? "Choose plan"}
                  </button>
                ) : buttonHref ? (
                ...
                {hasForfeitableTrial && !isTrialTier && t.id !== "free" ? (
                  <div className="pr-trial-warning">Changing plans ends your trial immediately and starts paid billing.</div>
                ) : null}
              </div>
            );
          })}
        ...
        <div className="pr-soft">
          ...
          <div>
            ...
            <div className="pr-soft-desc">Free workspaces stop accepting new publish requests after 100 posts/month and can hold up to 50 undeleted parent posts in scheduled status at once. On Basic and Growth, immediate publishing remains available during soft overage. New schedules pause when completed plus committed scheduled usage reaches 100%; cancel or move scheduled posts, wait for reset, or upgrade to restore capacity. Team has no monthly UniPost post quota. Platform safety limits, third-party API quotas, abuse controls, and shared-infrastructure fairness still apply.</div>
          </div>
        </div>
        ...
        {xCreditsEnabled ? <section className="pr-xcredits" aria-labelledby="x-credits-capacity">
          ...
          <div className="pr-xcredits-wrap">
            <table className="pr-xcredits-table">
              ...
              <tbody>
                {PUBLIC_X_CREDIT_PLANS.map((plan) => (
          ...
          <div className="pr-compare-wrap">
            <div className="pr-compare-hdr">
              ...
              {PUBLIC_TIERS.map((tier) => (
                <div key={tier.id} className={`pr-ch ${tier.id === "basic" ? "hl" : ""}`}>{tier.name}</div>
              ))}
            </div>
            {COMPARE_ROWS.map((row) => (
              <div key={row.name} className="pr-compare-row">
                ...
                {PUBLIC_COMPARE_PLAN_IDS.map((col) => {
                  const v = row[col];
                  return (
