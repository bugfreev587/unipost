...
export const PLATFORMS: Record<string, PlatformDoc> = {
  ...
  tiktok: {
    ...
    capabilities: [
      ...
      ["Analytics", partial, "Post engagement + view count require TikTok analytics scopes. Those analytics scopes are approved for production and requested when `tiktok.analytics_scopes` is enabled."],
    ...
    analytics: [
      ["Video views", partial, "Requires `video.list`; this scope is approved for production and requested when `tiktok.analytics_scopes` is enabled"],
      ["Likes", partial, "Requires `video.list`; this scope is approved for production and requested when `tiktok.analytics_scopes` is enabled"],
      ["Comments", partial, "Requires `video.list`; this scope is approved for production and requested when `tiktok.analytics_scopes` is enabled"],
      ["Shares", partial, "Requires `video.list`; this scope is approved for production and requested when `tiktok.analytics_scopes` is enabled"],
  ...
  "platform_options": {
    ...
    limitations: [
      ...
      ["Analytics scopes are approved", "Production OAuth requests `user.info.profile`, `user.info.stats`, and `video.list` when `FEATURE_TIKTOK_ANALYTICS_SCOPES` / `tiktok.analytics_scopes` is enabled. Disable the flag for emergency rollback."],
