...
1. Run `unipost doctor diagnose --json`. Parse `data.schema_version` (`doctor.v1`),
   `data.status`, and `data.findings`. Never scrape the human text.
...
3. To understand a specific failure, run
   `unipost doctor explain --request-id <id> --json` or
   `unipost logs list --status error --since 2h --json`.
4. When patching configuration files, prefer this order:
   1. Patch application code.
   2. Patch `.env.example`.
   3. Suggest changes to `.env`.
   4. Modify a real `.env` only after explicit user approval.
5. After every patch, run `unipost doctor verify --json`. A `data.status` of
   `passed` means the fix worked.
6. If verification still fails after a few bounded attempts, run
