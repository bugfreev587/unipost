...
- `api/internal/db/migrations/107_paid_soft_overage.sql`: quota-hold columns, paid notification ledger, follow-up table, indexes, and rollback.
...
**Files:**
- Create: `api/internal/db/migrations/107_paid_soft_overage.sql`
...
Add `TestPaidSoftOverageMigrationExists` to `api/internal/db/migrate_test.go`. Read migration 107 and assert it contains:
...
for _, required := range []string{
    ...
    if !strings.Contains(sql, required) {
        t.Fatalf("migration 107 missing %q", required)
    }
}
```
...
Expected: FAIL because migration 107 does not exist.

- [ ] **Step 3: Add migration 107**
...
Expected: generated Go files compile with migration 107 and all new queries.
