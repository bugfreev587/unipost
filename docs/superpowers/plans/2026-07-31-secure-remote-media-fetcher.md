...
- each new usage starts with `upload_state = pending` and a separate 15-minute `upload_lease_expires_at`, becomes active only after upload succeeds, and an expired upload lease becomes cleanup-eligible after an abandonment persistence failure;
- post-wide status/retention updates never mutate upload state or its lease, including when sibling jobs for the same post finish while an upload is pending;
- R2 upload has a 10-minute hard timeout, below the 15-minute upload lease;
...
- activation and cleanup serialize through the same object-row lock; two-session PostgreSQL tests cover activation-wins and cleanup-wins ordering, and activation fails closed after cleanup claims the object;
...
Attach server-owned workspace/post lifecycle metadata to the dispatch context. Reserve `publishing_pull_objects` plus a unique per-attempt `publishing_pull_object_usages` row atomically before `PutFile`, return its usage ID, and initialize it with `upload_state = pending` plus a database-clock 15-minute `upload_lease_expires_at`. These upload fields are independent from `post_status`, `cleanup_after_at`, and `retention_reason`; post-wide retention synchronization must not mutate them. Run `PutFile` with a 10-minute hard timeout. After upload, activate only that ID while holding the same object-row lock used by cleanup, clear the upload lease, and fail closed if the lease expired or cleanup already claimed the object. On failure, abandon only that ID. Both lifecycle writes use independent five-second bounded contexts derived with `context.WithoutCancel`. A failed abandonment write self-heals when the upload lease expires. Terminal post transitions update all usages for the post with `mediaretention.RetentionForPlanStatus`. Reservation, activation, and cleanup must contend on the same object row; cleanup first locks candidates, then checks usages in a separate Read Committed statement inside that transaction before marking an object deleting. The existing media cleanup worker deletes only claimed objects whose every usage is past deadline. This remains provider-neutral and introduces no Pinterest-specific retention window.
