...
Preview deployment note: Railway pull-request environments are disposable and begin with an empty PostgreSQL schema. The migration gate may bypass a backup only for a cryptographically identified exact application SHA in a matching `unipost-pr-<number>` environment whose matching Preview API URL, Goose version `0`, empty current schema, and zero affected-row counts are verified under the migration lock. Persistent development, staging, and production environments retain the original fail-closed backup requirement, including for zero-row pending irreversible migrations.
