...
Preview deployment note: Railway pull-request environments are disposable and begin with an empty PostgreSQL schema. The migration gate may bypass a backup only for a cryptographically identified exact application SHA in a matching `unipost-pr-<number>` environment whose Railway-provided public API domain is `preview-api-unipost-pr-<same-number>.up.railway.app`, Goose version is `0`, current schema is empty, and affected-row counts are all zero under the migration lock. Persistent development, staging, and production environments retain the original fail-closed backup requirement, including for zero-row pending irreversible migrations.
