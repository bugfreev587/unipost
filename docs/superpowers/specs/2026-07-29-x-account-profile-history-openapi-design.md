...
**Date:** 2026-07-29
**Status:** Approved in product-design review; implementation plan pending
...
4. Atomic X Credits preauthorization, actual-usage settlement, idempotent replay, and ambiguous-outcome reconciliation.
5. Workspace X Credits balance and transaction-ledger visibility.
6. Workspace, account, and managed-user isolation on every account read.
...
- Make every billable read idempotent and auditable.
- Reuse the existing X Credits catalog and ledger rather than create a second billing system.
...
- The account capability route already exists and can be extended additively.
...
### Mandatory Managed User selector

Every account-specific endpoint in this design requires exactly one nonempty `external_user_id` query parameter:

```text
GET /v1/accounts/{account_id}/capabilities?external_user_id=user_123
GET /v1/accounts/{account_id}/profile?external_user_id=user_123
GET /v1/accounts/{account_id}/posts?external_user_id=user_123&limit=100
```
...
- An account in the Workspace whose `external_user_id` differs from the request returns `403 ACCOUNT_ACCESS_DENIED`.
- A missing or empty `external_user_id` returns `400 VALIDATION_ERROR`.
- An account with a null `external_user_id` is not readable through these managed-account endpoints.
- A non-X account returns `400 WRONG_PLATFORM`.
...
This endpoint does not call X and does not consume Credits. It extends the existing account capability response with:
...
```json
{
  "data": {
    "profile_read": {
      ...
      "credit_operation": "user.read",
      "credits_per_resource": 10
    },
    "own_post_history_read": {
      "supported": true,
      "authorized": true,
      "reconnect_required": false,
      "credit_operation": "post.read",
      "credits_per_resource": 5,
      "min_page_size": 5,
      "max_page_size": 100
    }
  },
...
`Idempotency-Key` is required because the request can consume X Credits.
...
```text
GET /v1/accounts/{account_id}/threads/{conversation_id}
```
...
The cursor expires after 24 hours. A modified, expired, cross-account, cross-user, or filter-mismatched cursor returns `400 INVALID_CURSOR`. UniPost must not expose the upstream X pagination token directly.

Each continuation page requires a new `Idempotency-Key`. Replaying a completed page with the same key returns the stored response even if its cursor would otherwise have expired during the 24-hour receipt period.
...
Before any billable X call, UniPost calculates the maximum charge:
...
UniPost atomically reserves that amount against the Workspace's monthly allowance. Pending reservations count against available balance so concurrent reads cannot overspend.
...
- `scanned_count` is the number of post resources returned by X before UniPost filtering.
...
- Same key and same fingerprint while executing or reconciling: return `503 READ_SETTLEMENT_PENDING` with `Retry-After`; do not start a concurrent X call.
...
2. The client receives `503 READ_SETTLEMENT_PENDING`, `is_retriable: true`, and `Retry-After`.
...
This endpoint is Workspace scoped, does not call X, and consumes no X Credits. It supports cursor pagination and optional filters for:
...
P0 introduces a generic durable read-operation record, conceptually `x_read_operations`, with:

- operation ID;
- Workspace, social account, and `external_user_id` ownership;
- endpoint and canonical request fingerprint;
- a hash of the caller's idempotency key rather than the raw key;
- Credits usage-event reference;
...
- One Workspace and idempotency-key hash maps to at most one logical read operation during the receipt lifetime.
- The read operation and X Credits usage event are linkable for audit.
...
- Credits reservation and read-operation admission are committed before X is called.
- Final settlement and terminal operation state cannot diverge silently; failed finalization remains recoverable.
...
| 409 | `ACCOUNT_REAUTHORIZATION_REQUIRED` | Account is disconnected, token refresh failed permanently, or required scope is absent |
...
| 503 | `READ_SETTLEMENT_PENDING` | A same-key read is executing or has an ambiguous upstream outcome |
...
Credits availability is necessary but does not bypass operational limits. P0 applies bounded per-Workspace and per-account read concurrency and request-rate controls independently of the monthly allowance.
...
Every billable request emits structured fields sufficient to follow the operation without logging content:
...
The new profile, posts, and Credits-event routes are additive. The capability response and existing X Credits snapshot are extended additively.

No feature flag is required. API-layer changes default to direct rollout under the repository's established Preview Acceptance and environment-promotion gates.

The X Credits feature must already be available for the Workspace. A Workspace for which X Credits cannot be evaluated fails closed before a managed-app X read.

OpenAPI documentation must explain:

- the required `external_user_id` boundary;
...
- Missing `external_user_id`, null-owned accounts, and non-X accounts fail before invoking X.
...
- Profile preauthorizes one `user.read` unit.
- Posts preauthorize `limit * post.read` weight.
- Insufficient Credits returns 402 and the fake X client observes zero calls.
...
5. Verify same-key replay creates no second X call or Credits charge using the ledger.
6. Exercise an insufficient-Credits fixture or controlled allowance and prove zero upstream calls.
7. Verify the Workspace X Credits snapshot and event ledger match the completed operations.
8. Inspect sanitized logs for request IDs and operation IDs without profile text, post text, tokens, or raw cursors.
...
9. Insufficient Credits denies the complete page before X is called.
10. Successful reads settle against the actual number of upstream resources scanned.
