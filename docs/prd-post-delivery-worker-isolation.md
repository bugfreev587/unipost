...
- the worker waits for the whole claimed batch before claiming another batch
...
4. The dispatch worker does not claim more work until the batch drains far enough.
...
6. Add enough telemetry to measure queue latency, worker wait latency, platform latency, and drain rate.
7. Reduce long `publishing` states caused by worker-side backlog.
...
- Add a per-claim fairness rule such as:
  - no more than `N` newly claimed jobs per workspace per claim cycle, or
  - round-robin eligible workspaces, or
  - weighted fair scheduling by workspace plan in a later phase.
...
- Dispatch claim: maximum 2 newly claimed jobs per workspace per worker claim cycle.
- Retry claim: maximum 2 newly claimed jobs per workspace per worker claim cycle.
- Global batch size remains configurable.
...
- Continue using current state values, but derive display status from timestamps:
  - `pending` with no `last_attempt_at`: queued
...
- For every delivery job, operators can calculate:
  - queue wait: `last_attempt_at - created_at`
...
- Record timestamps:
  - `claimed_at` or reuse `last_attempt_at`
  - `platform_started_at`
  - `finished_at`
...
1. API service
   ...
   - enqueue
   - dashboard/API reads
   - lightweight background workers that are safe to keep in-process

2. Post delivery worker service
   - scheduler enqueue execution if needed
   - dispatch worker
   - retry worker
   - delivery cleanup
...
4. Worker marks claimed jobs with lease owner and claim timestamp.
5. Worker starts platform execution only when a local concurrency slot is available.
6. Worker writes `platform_started_at` immediately before adapter dispatch.
7. Worker writes terminal job state and refreshes parent post status after completion.
...
- choose up to `max_claim_per_workspace_per_cycle`
- order by oldest eligible job
- apply global batch limit
- keep same-account exclusion
- keep per-workspace active cap

This is not perfect weighted fair queuing, but it fixes the incident class with limited complexity.

### 6.4 Status model

Persist minimal timing data:

- `last_attempt_at`: claim timestamp
- `platform_started_at`: adapter start timestamp
- `finished_at`: terminal timestamp

Derived display:

| DB state | Timestamp condition | Derived phase |
| --- | --- | --- |
| `pending` | no `last_attempt_at` | `queued` |
| `running` | no `platform_started_at` | `reserved` |
| `running` | `platform_started_at` set | `dispatching` |
| `retrying` | no `platform_started_at` | `reserved_retry` |
| `retrying` | `platform_started_at` set | `retrying` |
| `succeeded` | terminal | `published` or result status |
| `failed` / `dead` | terminal | `failed` |
...
- Add `max_claim_per_workspace_per_cycle`.
...
3. Compare `last_attempt_at`, `platform_started_at`, and `finished_at`.
...
- make per-workspace claim cap configurable
- monitor drain rate
