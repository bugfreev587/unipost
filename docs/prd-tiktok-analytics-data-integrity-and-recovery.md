...
### 2.4 Recovery source integrity

UniPost stores TikTok's publish-operation ID in `social_post_results.external_id`. It does not store the rounded public video ID there.

The affected data flow is therefore:

```text
stored external_id = exact publish_id
  -> publish status resolves publish_id to public video ID
  -> current decoder rounds that public video ID in memory
```

The durable recovery source has not been corrupted. Historical recovery can replay each stored `publish_id` through the corrected publish-status decoder.

This recovery premise still depends on TikTok retaining the `publish_id` to public-video-ID mapping for the supported history window. Section 9.1 defines a mandatory age-boundary validation before UniPost promises full 90-day recovery.

### 2.5 Production impact snapshot
...
The zero `tiktok_video_id` count is another symptom of this incident, not evidence that the field is absent. The exact-ID success path writes `platform_specific.tiktok_video_id`, but the rounded lookup returned an empty `videos` array before that path could run. The adapter then returned an empty successful metric object, so the worker updated `fetched_at`, reset failure state, and preserved false zero values without a matched video ID.
...
- Do not redesign the analytics adapter interface to pass cached platform-specific IDs during the approximately two-hour incident fix.
...
### 6.3 Deferred hardening: reuse a cached public video ID

After a successful analytics match, `platform_specific.tiktok_video_id` can serve as a stable public-video identifier. A follow-up change should evaluate using that cached value for later refreshes instead of resolving the stored `publish_id` on every cycle.

This would reduce TikTok API calls and remove the long-term dependency on publish-status retention. It is deferred from the incident deployment because the current `AnalyticsAdapter.GetAnalytics` contract receives only `external_id`; cache-first behavior requires a broader worker/adapter data-flow change and migration-safe fallback tests.
...
The adapter must return a typed or wrapped classified error carrying at least the normalized reason and operation. Worker and handler classification must use the error type rather than substring matching. Tests must prove that no successful zero row is produced without a matched TikTok video.
...
| `social_accounts.status=reconnect_required`, or TikTok returns `access_token_invalid`, token expired, or equivalent | `NEEDS_RECONNECT` | `account_token_invalid` | 409 | reconnect account |
...
- `loadTikTokForAnalytics` must check `status` and `disconnected_at` before decrypting the token: disconnected state returns `ACCOUNT_DISCONNECTED`, while `status=reconnect_required` returns `NEEDS_RECONNECT` with `account_token_invalid`.
- `writeTikTokAnalyticsError` must emit the reason through `writeErrorWithDetails`; it must not collapse every authorization or provider failure into the current generic reconnect response.
- Register `ACCOUNT_DISCONNECTED`, `UPSTREAM_RATE_LIMITED`, `TIKTOK_TEMPORARY_ERROR`, and `TIKTOK_ANALYTICS_UNAVAILABLE` explicitly in `normalizedErrorCodeMap` so the supported contract is discoverable and typo-resistant.
...
This deliberately differs from the existing Pinterest analytics policy. Pinterest authorization failures currently mark the whole account `reconnect_required`. TikTok analytics-only scope failure must not copy that behavior because a TikTok token can remain valid for publishing while lacking `video.list`. Platform consistency must not override this capability difference.
...
- `ScopeReadiness` must not treat the stored requested-scope list as authoritative runtime proof. It may show an informational missing-scope state, but it may show `Reconnect required for analytics` only after the selected account's runtime response returns `analytics_scope_required`.
...
### 9.1 Recovery prerequisite: validate publish-status retention

Before UniPost commits to recovering the full 90-day window:

1. Select at least one accessible TikTok post as close as practical to the 90-day boundary, with a target age of 75-90 days.
2. Confirm its stored `social_post_results.external_id` is the original `publish_id`.
3. Call TikTok publish status with that `publish_id`.
4. Confirm TikTok still returns a public video ID and the corrected decoder preserves it exactly.
5. Query the returned public video ID and confirm TikTok returns the matching video.

If no suitable 75-90-day post exists, test the oldest available accessible post and record the maximum verified age.

If TikTok no longer resolves older `publish_id` values:

- do not block the incident deployment for current and recent posts;
- limit the promised recovery window to the oldest verified resolvable age;
- record older rows as explicit unavailable failures without overwriting existing real metrics; and
- revise customer communication before sending a historical-recovery estimate.

### 9.2 Eligible rows

Recovery includes published TikTok result rows, whether or not a `post_analytics` row already exists, when their related records meet all of:
...
3. Use a checked-in, version-controlled recovery runbook to mark the complete eligible set as due.
4. The runbook must use the existing refresh-endpoint pattern of setting only `post_analytics.fetched_at` to `1970-01-01 00:00:00+00`.
5. The runbook must not use `social_accounts.last_refreshed_at`; that field represents token-refresh state and is broader than this row-level recovery.
6. Do not delete `post_analytics` rows.
7. Do not clear metrics, `platform_specific`, `consecutive_failures`, or `last_failure_reason` while scheduling recovery.
8. Let `AnalyticsRefreshWorker` process the rows through its existing controls:
   - maximum 200 due rows per shared cross-platform cycle,
   - concurrency 5,
   - normal token refresh handling,
   - failure preservation.
9. Monitor each cycle until every eligible row has a post-deployment outcome.

The runbook must provide:

- a dry-run count grouped by account and post-age bucket;
- an explicit deployment timestamp input;
- one transaction for the scheduling mutation;
- an eligibility filter identical to Section 9.2;
- an idempotency guard that updates only rows whose current `fetched_at` is null or earlier than the deployment timestamp;
- inserted rows for eligible results that do not yet have `post_analytics`;
- a returned count of scheduled rows; and
- operator instructions to stop before commit if the mutation count differs from the dry run.

Rerunning the runbook with the same deployment timestamp must not re-arm rows that already received a post-deployment outcome. The runbook must be executed by an authorized production operator only after the fixed deployment passes the first production metric check.

The existing workspace refresh endpoint is not sufficient as the only global recovery mechanism because it caps requests, selects the newest matching rows, and cannot address every workspace in one controlled operation.

### 9.4 Recovery completion definition
...
### 9.6 Estimated duration

Using the investigation snapshot:

```text
2,126 rows / at most 200 rows per shared cycle = at least 11 worker cycles
```

The first cycle runs at application startup and later cycles run hourly. The 200-row limit is shared across all platforms, so the actual TikTok throughput must be measured from each cycle rather than assumed to be 200. Other due platforms and provider retries can extend the duration.
...
10. Classified adapter errors preserve their reason through wrapping without relying on error-message substring matching.
...
7. An account with `status=disconnected` or a valid `disconnected_at` is rejected before token decryption with `ACCOUNT_DISCONNECTED` and `details.reason=account_disconnected`.
8. An account with `status=reconnect_required` is rejected before token decryption with `NEEDS_RECONNECT` and `details.reason=account_token_invalid`.
9. TikTok analytics-only scope failure does not call `MarkSocialAccountReconnectRequired`; the existing Pinterest-specific behavior remains unchanged.
...
8. Stored missing scopes alone do not make `ScopeReadiness` show `Reconnect required`; the runtime `analytics_scope_required` response does.
...
9. The recovery runbook dry run and execute paths select only eligible TikTok rows, preserve metric and failure columns, and do not re-arm a row with a post-deployment outcome.
...
2. Complete the Section 9.1 publish-status retention check and record the maximum verified recoverable age.
3. Verify the known precision reproduction no longer rounds the video ID.
4. Verify at least one known customer video returns non-zero real metrics when TikTok currently reports non-zero values.
5. Confirm healthy `clipsworld566` does not require reconnect.
6. Confirm disconnected `clippingstudiox` remains excluded until user reconnection.
7. Run the recovery runbook dry run, reconcile its count, then mark eligible rows due.
8. Monitor the controlled recovery and effective TikTok rows processed per shared worker cycle.
9. Confirm every eligible row reaches a post-deployment success or explicit failure outcome within 12-24 hours.
10. Confirm TikTok publish success and failure rates do not regress during recovery.
...
19. The maximum recoverable publish age is verified before promising a 90-day recovery window.
20. The recovery scheduler is idempotent for a fixed deployment timestamp and preserves all existing metric and failure data while marking rows due.
21. Stored requested scopes alone cannot trigger the dashboard's reconnect-required state.
...
6. Validate the oldest available authorized `publish_id` mapping and set the supported recovery promise from the verified age.
7. Promote through the approved staging and production release path when production release is authorized.
8. Target production incident deployment approximately two hours after implementation is approved and started.
9. Verify production health and one known TikTok metric before marking historical rows due.
10. Run and reconcile the recovery runbook dry run, then start the controlled historical recovery.
11. Monitor provider errors, publish health, shared-worker TikTok throughput, recovery progress, and customer-visible metrics.
12. Send the customer completion update only after the deployment is healthy and the historical recovery has reached its completion criteria.
...
- provider-resolvable historical data will recover progressively within approximately 12-24 hours after deployment.

Promise a full 90-day recovery window only after the Section 9.1 age-boundary check succeeds. Otherwise, state the maximum verified recoverable age and explain that older unavailable rows will be identified rather than represented as zero.
...
5. TikTok intentionally does not inherit Pinterest's analytics-auth account-level reconnect policy.
6. The stored TikTok `external_id` remains an exact `publish_id`, so the durable recovery source is intact.
7. Historical recovery uses the existing worker's bounded batches and concurrency.
8. The target recovery window is 90 days, conditional on the provider age-boundary validation.
9. Recovery scheduling uses the existing epoch `fetched_at` pattern through an idempotent, version-controlled operator runbook.
10. There is no feature flag and no new database table.
11. Cache-first reuse of `tiktok_video_id` is a post-incident hardening item, not part of the approximately two-hour deployment.
12. Implementation does not begin until this PRD is reviewed and approved.
