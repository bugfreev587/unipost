...
Preview Acceptance requires the repository secrets `VERCEL_TOKEN` and `RAILWAY_API_TOKEN`, plus the repository variable `RAILWAY_PROJECT_ID`. Each run uploads its prebuilt Vercel output as a compressed archive, creates a unique Vercel alias for signed-in human inspection, and creates a 24-hour Shareable Link for the immutable Vercel deployment URL through Vercel's API. Playwright uses that link only to establish the deployment-scoped access cookie, so Preview deployments remain protected from unauthenticated visitors and concurrent or repeated runs cannot overwrite one another. `RAILWAY_API_TOKEN` must be a dedicated Railway workspace token so the workflow can resolve the ephemeral environment ID from Railway's GitHub Deployment record, trigger that environment's `preview-api` when Railway skips an unchanged monorepo path, and read its domain and exact deployed commit. Do not use a production project token or a personal account-wide token.
