...
Before activation, a dedicated activation PR must wire a bounded asynchronous `OnPersistedBatch` canonical WebSocket publisher and prove that the callback cannot delay or backpressure request-event persistence. It must also prove that ON delivers exactly one canonical request event, OFF and SSE preserve numeric no-gap delivery, and selector failure falls back to legacy. The current WebSocket projection contains an explicit `TODO(activation)` at the intentional legacy `api_request` drop boundary; that TODO is not satisfied by a synchronous callback.

Metrics v2 responses expose source quality through the top-level `freshness` object, while `meta` remains reserved for the shared pagination schema. Admin API Metrics must visibly distinguish delayed results and histogram-bound approximate percentiles before activation. Only after those UI semantics, the live-stream safety gate, production-like query-plan evidence, and the four-SDK matrix pass may exact-SHA Preview and deployed dev acceptance unlock `ActivationReady`. There is no third-party runtime dependency for this switch.
