...
async function uploadMediaToLibrary(apiKey, args) {
    if (!args.base64_data && !args.url) {
        return { error: "Error: pass either base64_data or url." };
    }
    let sizeBytes;
    let bytes = null;
    if (args.base64_data) {
        ...
        if (!sizeBytes) {
            return {
                error: "Error: could not determine size of remote URL via HEAD, so UniPost cannot reserve a /v1/media upload with size_bytes > 0. " +
                    HOSTED_URL_GUIDANCE,
            };
        }
    }
    ...
    if (!created?.data?.upload_url) {
        ...
        return {
            error: "Error: API did not return an upload URL." +
                apiMessage +
                " " +
                HOSTED_URL_GUIDANCE +
                " Raw API response: " +
...
function createMcpServer(apiKey) {
    ...
    server.tool("unipost_get_media", "Get a media row by id. Use this to confirm status=uploaded before publishing or to inspect the hydrated media metadata after upload.", {
        media_id: zod_1.z.string().describe("Media library id returned by unipost_upload_media."),
    }, async ({ media_id }) => {
        const data = await apiRequest(`/v1/media/${media_id}`, apiKey);
        return { content: [{ type: "text", text: JSON.stringify(data.data, null, 2) }] };
    });
    server.tool("unipost_create_youtube_video_post", [
        ...
        account_id: zod_1.z.string().describe("YouTube social account id."),
        caption: zod_1.z.string().optional().describe("Caption or description context."),
        media_id: zod_1.z.string().optional().describe("Existing UniPost media id for the uploaded video."),
        video_url: zod_1.z.string().optional().describe("Publicly fetchable video URL to upload and publish."),
        filename: zod_1.z.string().optional().describe("Required when base64_data is set."),
        content_type: zod_1.z.string().optional().describe("Required when base64_data is set. Usually video/mp4."),
        base64_data: zod_1.z.string().optional().describe("Base64-encoded video body for smaller local files."),
        privacy_status: zod_1.z.enum(["private", "public", "unlisted"]).optional(),
        shorts: zod_1.z.boolean().optional(),
        ...
        if (!mediaId) {
            ...
            if ("error" in uploaded) {
                return {
                    content: [{ type: "text", text: uploaded.error }],
                };
            }
    ...
    server.tool("unipost_get_managed_user", "Get the detail view of one managed end user: every social account they've connected, with per-account status and platform info. Pair with unipost_list_managed_users to walk a workspace's end-user list.", {
    ...
    server.tool("unipost_update_workspace_quota", [
